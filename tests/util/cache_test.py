...
def get_cache_dir(no): return get_resource(
    __file__, ('.unittest-temp%d' % (no), ))
...
class TestCached(unittest.TestCase):
    ...
    def testNonKeywordArguments(self):
        ...
        for x in range(1, 20):
            self.assertEqual(self.add(x, 5), (x + 5))
            self.assertEqual(self.add(x, 5), (x + 5))

        # test objects with a cachesize specified
        for x in range(1, 20):
            self.assertEqual(self.sub(x, 5), x - 5)
            self.assertEqual(self.sub(x, 5), x - 5)
    ...
    def testKeywordArguments(self):
        ...
        self.assertEqual(self.add(3, b=7), (3 + 7))
        self.assertEqual(self.add(3, b=7), (3 + 7))
        self.assertEqual(self.add(a=9, b=8), (9 + 8))
...
#
#         def getCacheLocation(x): return join(CACHE_DIR, Cache.getObjectId(x))
#
...
#
#         def getTestIterator(x): return list(range(x))
#
...
def f(c):
    ...
    def blow(x): return x not in (7, 8) and 100000 * str(x) or None
