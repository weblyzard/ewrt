...
strCleanupPipe = (lambda s: s.replace('\xd7', " "),
                  str.lower, RemovePossessive(), FixDashSpace())
...
def computeStatistics(ff):
    ...
    for f in ff:
        concepts = set(map(str, extractConceptSet(
            XCL2RDF.toRDF(open(f).read()))))
        relations = set(
...
def computeOntologyStatistics(ff, cc, rc, ccCutOffCount, rcCutOffCount):
    ...
    for f in ff:
        ...
        cPrecision = len(goldStandardConcepts.intersection(
            concepts)) / float(len(concepts))
        cRecall = len(goldStandardConcepts.intersection(
            concepts)) / float(len(goldStandardConcepts))
        if (cPrecision + cRecall) == 0.:
            cF1 = "NaN"
        else:
            cF1 = 2 * cPrecision * cRecall / (cPrecision + cRecall)

        rPrecision = len(goldStandardRelations.intersection(
            relations)) / float(len(relations))
        rRecall = len(goldStandardRelations.intersection(
        ...
        w.writerow((path.basename(f), cPrecision, cRecall,
                    cF1, rPrecision, rRecall, rF1))
