...
RELATION_PREDICATES = ('rdfs:subClassOf', 'wl:isRelatedTo',
                       'wl:modifierOf', 'wl:social', 'wl:deleted')

# ontology cleanup
from eWRT.input.clean.text import *

strCleanupPipe = (lambda s: s.replace('\xd7', " "),
...
class OutputQueries(object):
    ...
    def _webLyzardSparqlQuery():
        constraints = ["{ ?c1 %s ?c2; ?p ?c2; rdfs:label ?s. ?c2 rdfs:label ?o. }" %
                       p for p in RELATION_PREDICATES]
...
class Output(object):
    ...
    def __init__(self, ontology, sparqlQuery=OutputQueries._webLyzardSparqlQuery):
        ...
        """
        self.ontology = ontology if isinstance(
            ontology, Graph) else Graph().parse(ontology)
    ...
    def getOntologyStatements(self):
        ...
        for ss, pp, oo in self.ontology.query(q, initNs=dict(rdfs=NS_RDFS, wl=NS_WL)):
            rel = rel.union([(s, p, o) for s in phraseCleanup.clean(
                ss) for p in phraseCleanup.clean(pp) for o in phraseCleanup.clean(oo)])
        return rel
...
class GraphvizVisualize(Output):
    ...
    WL_PREDICATE_MAPPING = {'subClassOf': "[]",
                            'isRelatedTo': '[style="dashed", dir="none", label="r"]',
                            'modifierOf':  '[style="dashed", label="m"]',
                            'deleted':  '[style="dotted", arrowhead="invempty"]',
                            'social':  '[style="bold", dir="none"]'
                            }
    GRAPHVIZ_HEADER = """
        rotate="90"
        size="10.8,7.4"
    ...
    def __str__(self):
        ...
        relationDefinitions = ['"%s" -> "%s" %s' %
                               (s, o, self.mapPredicate(p)) for s, p, o in stmts]
    ...
    def createImage(self, fname, imgFormat):
        ...
        getoutput('fdp -T%s %s -Elen=0.1 -Eweight=3  -Gmindist=0.05 -Nmargin="0.0,0.0" -Nfontname="Helvetica" -Nheight=0.6 -Nwidth=1.1 -Nfontsize=15 -Goverlap="4:" -Goutputorder="edgesfirst" -Gsplines="true" -Gratio=0.7 -o%s.%s' %
                  (imgFormat, dotFile.name, fname, imgFormat))
